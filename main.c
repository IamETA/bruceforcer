...
void bruceforce_bruteforce(const char* hash, char *salt, int size, int THREADS);
...
void bruceforce_bruteforce(const char* hash, char *salt, int size,int THREADS) {
  ...
  char *charactertable="abcdefghikjlmnopqrstuvwxyz";//ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890 !#$%&'()*+,-./:;<=>?@[\\]^_`{|}~\"\0";
...
void* bruteforceThreadCallback(void* args) {
  ...
  printf("Bruteforce Thread %i exited. ",context->id);
  if (bf_result==NULL) {
    printf("No results");
  }
  else {
